...
exports.findAll = async (req, res) => {
  try {
    const [data] = await db.query("SELECT * FROM Jeu");
    res.send(data);
  } catch (err) {
...
exports.create = async (req, res) => {
  ...
  try {
    await db.query(
      "CALL AjouterJeu(?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [nom_jeu, photo, description, annee_sortie, mini_player, max_player, duree_mini, duree_max, categorie]
    );
...
exports.findOne = async (req, res) => {
  try {
    const [data] = await db.query("SELECT * FROM Jeu WHERE Id_jeu = ?", [req.params.id]);
    res.send(data[0]);
  } catch (err) {
    res.status(500).send({ message: err.message });
  }
...
exports.update = async (req, res) => {
  ...
  try {
    await db.query(
      "CALL ModifierJeu(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [id, nom_jeu, photo, description, annee_sortie, mini_player, max_player, duree_mini, duree_max, categorie]
    );
...
exports.delete = async (req, res) => {
  try {
    await db.query("DELETE FROM Jeu WHERE Id_jeu = ?", [req.params.id]);
